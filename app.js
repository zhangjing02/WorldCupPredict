...
function togglePaid(id) {
  ...
  if (bet) { bet.paid = !bet.paid; renderLedger(); }
}
  ...
  try {
    ...
    if (raw.startsWith('{') && raw.endsWith('}')) {
      ...
      if (Array.isArray(list)) {
        ...
        list.forEach(b => {
          b.id = state.nextId++;
          b.time = b.time ? new Date(b.time) : new Date();
          state.bets.unshift(b);
          addedCount++;
        });
        closeModal('importModal');
        renderLedger();
        showToast(`📥 成功批量导入并汇总 ${addedCount} 笔投注记录！`, '✅');
        return;
      }
    }
    else {
    ...
    if (!betObj || !betObj.name || !betObj.type || !betObj.teams || !betObj.amount) {
    ...
    const bet = {
      ...
      name: betObj.name,
      teams: betObj.teams,
      amount: Number(betObj.amount),
      payout: Number(betObj.payout),
      profit: Number(betObj.profit),
      paid: betObj.paid || false,
      time: betObj.time ? new Date(betObj.time) : new Date()
    };
    ...
    if (isDup) {
      document.getElementById('importErr').textContent = '⚠️ 这笔注单已被导入过，请勿重复操作！';
      return;
    }
    ...
    closeModal('importModal');
    renderLedger();
    showToast(`📥 成功汇总下注单：${bet.name} (¥${bet.amount})`, '✅');

  } catch (err) {
    document.getElementById('importErr').textContent = '❌ 解析失败！请确保复制并粘贴了包含密文的完整分享消息。';
  }
}
    ...
    <tbody>
      ${state.bets.map(b => {
        ...
        return `<tr>
          ...
          <td style="font-size:11px;color:var(--muted)">${fmt(b.time)}</td>
          <td>${paidBadge}</td>
          ${delBtn}
        </tr>`;
      }).join('')}
