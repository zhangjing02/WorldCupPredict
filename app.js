...
function toggleTeam(type, name) {
  ...
  if (s.has(name)) { s.delete(name); }
    ...
    if (s.size >= max) {
      if (max===1) { s.clear(); }
      else { showToast(`已选满 ${max} 支球队！请先取消其他球队的选择。`, '⚠️'); return; }
    }
    s.add(name);
  }
...
function updateCount(type) {
  ...
  clearEl.style.display = count > 0 ? 'inline' : 'none';
}
...
function updatePayout(type) {
  ...
  if (rawVal !== '') {
    ...
    if (num < 1) { inputEl.value = ''; state[type].amount = null; }
    else { inputEl.value = num; state[type].amount = num; }
  } else {
...
function copyShareText() {
  ...
  navigator.clipboard.writeText(lastSubmittedBetShareText).then(() => {
    ...
    ta.select();
    try { document.execCommand('copy'); showToast('📋 注单分享文本已复制！快去发送给庄家付款吧~', '✅'); }
    catch(e) { showToast('❌ 自动复制失败，请截图或手动复制。'); }
...
async function togglePaid(id) {
  ...
  if (bet) {
    ...
    if (!success && useRemoteApi) { bet.paid = !nextPaidState; renderLedger(); }
  }
}
...
async function markAllPaid() {
  ...
  if (success) { showToast('✅ 已将当前所有注单标记为已付款！'); }
  else if (useRemoteApi) { state.bets.forEach((b, idx) => b.paid = backup[idx]); renderLedger(); }
}
...
async function markAllUnpaid() {
  ...
  if (success) { showToast('⭕ 已重置所有注单为未付款！'); }
  else if (useRemoteApi) { state.bets.forEach((b, idx) => b.paid = backup[idx]); renderLedger(); }
}
...
async function submitImport() {
  ...
  try {
    ...
    if (raw.startsWith('{') && raw.endsWith('}')) {
      ...
      if (Array.isArray(list)) importList = list;
    } else {
      const match = raw.match(/------------ 庄家导入密文 \(请勿修改\) ------------\s*(\{[\s\S]*?\})\s*----------------------------------------------/);
      if (match && match[1]) betObj = JSON.parse(match[1]);
    }
    if (!importList.length && betObj) importList = [betObj];
    if (!importList.length) throw new Error('无效的注单格式');
    ...
    importList.forEach(item => {
      const bet = {
        id: state.nextId++,
        type: item.type, name: item.name, teams: item.teams,
        amount: Number(item.amount), payout: Number(item.payout), profit: Number(item.profit),
      ...
      if (isDup) duplicatedNames.push(bet.name);
      else newBetsToInsert.push(bet);
    });
    ...
    if (newBetsToInsert.length > 0) {
      ...
      if (success) { showToast(`📥 成功导入并汇总 ${newBetsToInsert.length} 笔投注记录！`, '✅'); }
      else if (useRemoteApi) {
        newBetsToInsert.forEach(b => { const idx = state.bets.findIndex(sb => sb.id === b.id); if (idx !== -1) state.bets.splice(idx, 1); });
        renderLedger();
        showToast('❌ 同步云端数据库失败！', '❌');
      }
    }
...
function openExportModal() {
  if (!state.bets.length) { showToast('⚠️ 当前无投注记录，无法导出！', '⚠️'); return; }
...
async function deleteBet(id) {
  ...
  if (i !== -1) {
    ...
    if (success) { showToast(`🗑️ 已成功撤销 ${name} 的注单`); }
    else if (useRemoteApi) { state.bets.splice(i, 0, backupBet); renderLedger(); }
  }
}
...
function renderLedger() {
  ...
  const adminCols = state.adminUnlocked ? '<th>付款状态</th><th>操作</th>' : '<th>付款状态</th>';
